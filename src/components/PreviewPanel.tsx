...
const QUICK_URLS = [
  ...
  { label: "Node.js",            url: "http://localhost:3005"  },
...
export default function PreviewPanel({ previewUrl, isLoading }: PreviewPanelProps) {
      ...
      <div className="flex-1 overflow-auto bg-[#2d2d2d] flex items-start justify-center p-2">
        {isLoading ? (
          <div className="flex-1 h-full flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm text-[#858585]">Booting WebContainer...</p>
            <p className="text-xs text-[#444]">This may take a few seconds</p>
          </div>
        ) : displayUrl && isLocalhost(displayUrl) && displayUrl !== previewUrl ? (
